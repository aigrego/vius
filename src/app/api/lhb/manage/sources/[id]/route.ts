...
import { deleteLhbSource, updateLhbSource } from '@/model/LhbSource';
...
// PUT /api/lhb/manage/sources/[id] - 编辑数据源（仅 admin，含启停 enabled）
export const PUT = async (request: NextRequest, ctx: { params: Promise<{ id: string }> }) => {
  ...
  try {
    ...
    if ('apiKey' in body) data.apiKey = opt(body.apiKey, 255);
    if ('cron' in body) data.cron = opt(body.cron, 50);
    ...
    const updated = await updateLhbSource(id, data);
    ...
    console.error('[api/lhb/manage/sources/[id]] 更新失败:', error);
    return NextResponse.json({ code: 500, data: null, message: '更新失败' }, { status: 500 });
  }
};

// DELETE /api/lhb/manage/sources/[id] - 删除数据源（仅 admin，不影响已落库的龙虎榜数据）
export const DELETE = async (_request: NextRequest, ctx: { params: Promise<{ id: string }> }) => {
  ...
  try {
    ...
    await deleteLhbSource(id);
    return NextResponse.json({ code: 200, data: { id }, message: '已删除' });
  } catch (error: any) {
    if (error?.code === 'P2025') {
      return NextResponse.json({ code: 404, data: null, message: '数据源不存在' }, { status: 404 });
    }
    console.error('[api/lhb/manage/sources/[id]] 删除失败:', error);
